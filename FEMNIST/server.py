...
def main() -> None:
    ...
    if (args.model == "Net"):
        ...
        _, testset = utils.load_femnist(evaList)
    else:
        _, testset = utils.load_cifar(download=True)

    # Create client_manager, strategy, and server
    client_manager = fl.server.SimpleClientManager()
    strategy = fl.server.strategy.FedDrop(
...
def fit_config(rnd: int, p=1.0, batch=args.batch_size, epochs=1) -> Dict[str, fl.common.Scalar]:
