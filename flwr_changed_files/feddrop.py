...
class FedDrop(Strategy):
    ...
    def configure_fit(
        ...
        for client in clients:
            if (client.cid in self.straggler) and rnd > 1:
                ...
                fit_ins_drop = FitIns(self.drop_order(parameters, p_val, [0,4], 2, client.cid), config_drop)
    ...
    def aggregate_fit(
        ...
        self.find_stable(self.parameters, weights_results, [0,4], 2)

        # find minimum weight changes to initialize threshold (only done once)
        self.find_min(self.parameters, weights_results, [0,4], rnd)
    ...
    def find_stable(self, parameters: Parameters, results: List[Tuple[Weights, int, str]], idxList: List[int], idxConvFC: int):
        ...
        for idx in idxList:
            ...
            print("threshold: ", self.changeThreshold)
        ...
        for i in range(len(idx0Layer)):
            reducedList = np.array(np.append(reducedList, np.all(idx2Layer[i*7*7 :  i * 7 * 7 + 7 * 7])), dtype=bool)
