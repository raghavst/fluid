...
class ShakeClient(fl.client.Client):
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        if (p != self.p):
            ...
            self.p = p
            self.model = utils.load_model("Shakespeare_LSTM", self.p)
    ...
    def evaluate(self, ins: EvaluateIns) -> EvaluateRes:
        ...
        loss, accuracy = utils.test_shake(self.model, testloader, device=self.device)
...
def main() -> None:
    ...
    trainset, testset = utils.load_shake([args.cid])

    # Start client
    client = ShakeClient(args.cid, model, trainset, testset, device)
