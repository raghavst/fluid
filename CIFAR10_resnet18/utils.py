...
def train(
    ...
    print("Initial weights check:", np.count_nonzero(initial_weights[0]), "non-zero elements in the first layer's weights.")
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for i, (images, labels) in enumerate(trainloader, 0):
            ...
            inputs_list.append(images.cpu().numpy())
    ...
    print("Post-training weights check:", np.count_nonzero(final_weights[-1]), "non-zero elements in the first layer's weights.")
    return initial_weights[-1], final_weights[-1], inputs_array
